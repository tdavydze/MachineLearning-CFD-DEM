...
defineTypeNameAndDebug(DiFeliceDrag_NN, 0);
...
DiFeliceDrag_NN::DiFeliceDrag_NN
(
...
void DiFeliceDrag_NN::setForce() const
{
            ...
            if (cellI > -1) // particle Found
            {
                ...
                if (magUr > 0)
                {
                   ...
                   /* dragCoefficient = 0.125*Cd*rho
                    ...
                    in.data_[4] = Ur[1]*100;
                    in.data_[5] = Ur[2]*100;
                    in.data_[6] = voidfraction;
                    
                    keras2cpp::Tensor_keras out = model(in);
                    out.print_dragX();
                    out.print_dragY();
                    out.print_dragZ();
                    drag[0] = dragX/1000000;
                    drag[1] = dragY/1000000;
                    drag[2] = dragZ/1000000;



    //Info << "\n Us " << Us << "   Ur "<< Ur  << "   void "<< voidfraction  << "   drag "<< drag << nl << endl;
                    // explicitCorr
                    for (int iFSub=0;iFSub<nrForceSubModels();iFSub++)
                ...
                if(forceSubM(0).verbose() && index >-1 && index <102)
                  //if(forceSubM(0).verbose() && index == 100)
                {
